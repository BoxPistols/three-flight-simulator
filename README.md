...
- 🗾 **実在都市モード** — [Project PLATEAU](https://www.mlit.go.jp/plateau/)（国土交通省）の実建物モデルと国土地理院の実地形・空中写真の上でフライト計画。東京駅・秋葉原・皇居周辺のプリセットに加え、PLATEAU の 3D Tiles 配信URLを指定して任意の都市を追加可能
...
サイドパネルの「環境」で **実在都市** を選ぶと、国土地理院の標高タイル・空中写真による実地形と、Project PLATEAU の建築物モデル（3D Tiles）が読み込まれます。

- 地表クリックで追加されるウェイポイントの高度は **地表からの高さ（AGL）** として扱われます
- プランのエクスポートJSONには原点の緯度経度（`origin`）が含まれ、ジオリファレンスされます
- 他都市を使う場合は「カスタム地点を追加」から PLATEAU データカタログ（plateauview.mlit.go.jp）で確認した tileset.json URL を指定してください
- タイルはブラウザから直接取得されます（出典: 国土地理院・国土交通省 Project PLATEAU）

### 座標系

シーン座標(メートル単位)を使用します: **X = 東西 [m]、Z = 南北 [m]、高度 [m]**。1シーン単位 = 1m。実在都市モードでは選択ロケーションの中心が原点(+X=東、+Z=南)になります。
...
- **3d-tiles-renderer** — PLATEAU 建築物モデル（3D Tiles）の読み込み
- **Material-UI v7 + Emotion** — UI
- **Zustand** — 状態管理（localStorage 永続化 + Undo/Redo 履歴）
- **Vitest** — ユニットテスト

データソース（実在都市モード・ブラウザから直接取得）:
- 地形・空中写真: [国土地理院 地理院タイル](https://maps.gsi.go.jp/development/ichiran.html)（標高タイル dem_png / 全国最新写真）
- 建築物: [Project PLATEAU](https://www.mlit.go.jp/plateau/)（国土交通省）3D Tiles 配信
